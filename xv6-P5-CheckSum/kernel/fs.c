...
bmap(struct inode *ip, uint bn)
{
  ...
  if (ip->type == T_CHECKED) {
	if (bn < NDIRECT) {
		if ((addr = clearByte(ip->addrs[bn])) == 0) {
			ip->addrs[bn] = addr = clearByte(balloc(ip->dev));
		}
		return addr;
	}
	bn -= NDIRECT;
	if (bn < NINDIRECT) {
		if ((addr = clearByte(ip->addrs[NDIRECT])) == 0) {
			ip->addrs[NDIRECT] = addr = clearByte(balloc(ip->dev));
		}

		bp = bread(ip->dev, addr);
		a = (uint*) bp->data;
		if ((addr = clearByte(a[bn])) == 0) {
			a[bn] = addr = clearByte(balloc(ip->dev));
			bwrite(bp);
		}
		brelse(bp);
		return addr;
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
	...
	if (ip->type == T_CHECKED) {
		...
		brelse(bp);
		if (bn < NDIRECT) {
			if (sum != getSum(ip->addrs[bn])) {
				return -1;
			}
		} else {
			bn -= NDIRECT;
			bp = bread(ip->dev, clearByte(ip->addrs[NDIRECT]));
			a = (uint*) bp->data;
			brelse(bp);
			if (sum != getSum(a[bn])) {
				return -1;
...
{
  uint tot, m;
  struct buf *bp; //buffer cache?
  uint *a;
  int bn;

  //other device code, ignore///////////////////////////////////////////////
  if(ip->type == T_DEV){
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
	...
	if (ip->type == T_CHECKED) {
		...
		if (bn < NDIRECT) {
			ip->addrs[off/BSIZE] = genCheckedAddr(sum, ip->addrs[off/BSIZE]);
			brelse(bp);
		} else {
			bn -= NDIRECT;
			brelse(bp);
...
//clears the leftmost byte
uint clearByte(uint num) {
	return (num & 0x00ffffff);
}

uint genCheckedAddr(uchar sum, uint addr) {
	//zero extend to the right
	//clear the leftmost byte
	//combine the two
	return (sum << 24) | clearByte(addr);
}

uchar getSum(uint addr) {
